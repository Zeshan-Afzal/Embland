...
function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/user-upd/update/${currentUser._id}`, {
        method: "POST",
        headers: {
  ...
  const hanldeClick = async () => {
    try {
      ...
      const res = await fetch(`/api/user-upd/delete/${currentUser._id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success === "false") {
  ...
  const handleShowUserListings = async () => {
    try {
      setTabOpen("listing");
      const res = await fetch(`/api/user-upd/user-listings/${currentUser._id}`);
  ...
  const handleDeleteListing = async (id) => {
    console.log(id);
    try {
      const res = await fetch(`/api/user-upd/delete-listing/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className=" flex pt-2 md:p-2 gap-3">
      <div className="  flex flex-col w-16 md:w-56 border  border-gray-300 rounded-lg gap-0 shadow-lg">
        ...
        <div className=" md:justify-normal justify-center flex items-center  gap-1  mt-8 hover:bg-slate-200 p-2 rounded-lg transition-all duration-200  cursor-pointer">
          <IoMdCreate className=" text-2xl font-bold text-blue-600"></IoMdCreate>
          <Link
            className=" text-xl font-semibold md:inline hidden text-blue-600"
            to="/create-listing"
          >
            Create
          </Link>
